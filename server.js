...
app.use('/web/static', express.static(path.join(__dirname, '/web/static')));
...
class UIServer {
  ...
  runConfig(config) {
    if (!this.isRunningConfig && !this.childProcess) {
      ...
      this.childProcess.on('message', (msg) => {
        //console.log(`this.childProcess.on('message'`);
        if (msg.results) {
      ...
      this.childProcess.on('close', () => {
        //console.log(`this.childProcess.on('close'`);
        this.isRunningConfig = false;
        this.currentConfig = null;
        this.history = null;
      });
  ...
  async storeHistory(test) {
    ...
    if (isTopFifty) {
      fs.writeFile(historyFile, JSON.stringify(this.history), { encoding: 'utf8' }, (err) => {
        if (err) throw err;
...
app.post('/run-config', async (req, res) => {
  console.log(`API.POST: run-confg`)
